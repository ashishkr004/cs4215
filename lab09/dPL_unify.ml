...
let rec eq_type t1 t2 =
  match t1,t2 with
    ...
    (* unifying two types *)
let rec unify_aux (t1:dPL_type) (t2:dPL_type) 
...
let unify_type t1 t2 = 
  ...
  Debug.ho_2 "unify_type" pr pr (pr_pair pr_subs pr) unify_type t1 t2
...
let _ = unify_test (TDef ("list",[TVar "d"]))  (TDef ("list",[TError])) (* 15 *)
