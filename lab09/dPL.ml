...
let rec exists_constr name n lst = 
  match lst with
    | []         -> (-1, false)
    | (nm,_)::tl -> if (nm = name) then (n,true) else exists_constr name (n+1) tl

let rec find_constr name lst = 
  match lst with
    | []        -> raise Not_found
    | (_,d)::tl -> 
          let (tag_no, exists) =  exists_constr name 0 d.ty_def_data in
...
object (self)
  ...
  (* method find_tag c : dPL_type_decl  = *)
  (*   (\* find a constructor tag *\) *)
  (*   try *)
  (*     let m = List.find (fun (ix,d) ->  *)
  (*          List.exists (fun (i,  _) -> i=c) d.ty_def_data) dict  *)
  (*     in (snd m) *)
  (*   with _ -> raise Not_found *)

  method find_tag c : int * dPL_type_decl  =
    (* find a constructor tag *)
    try
       find_constr c dict
    with _ -> raise Not_found
  ...
  method  find_tag_in_dt (c:id) (dt: dPL_type_decl) :  dPL_type list =
    (* find a constructor tag *)
    try
...
type dPL_expr =
  ...
  | Match of dPL_expr * (id * (id list) * dPL_expr) list
        (* pattern-matching *)
  | Func of dPL_type option * (id list) * dPL_expr 
        (* min of one parameter *)
  | RecFunc of dPL_type option * id * (id list) * dPL_expr 
        (* min of one parameter *)
  | Appln of dPL_expr * dPL_type option * (dPL_expr list) 
        (* at least one argument *)
  | Let of ((dPL_type option * id * dPL_expr) list) * dPL_type option * dPL_expr
...
let string_of_dPL (e:dPL_expr):string =
  ...
  match e with
    ...
    | Match (e1,lst) -> "\nmatch "^(aux e1)^" with \n"^(pr_lst ";\n"
          (fun (i,vs,e)-> i^" "^(pr_lst " " pr_id vs)^" -> "^(aux e)) lst)^" end \n"
    | Let (lst,t,body) -> 
...
let rec fv (e:dPL_expr) : id list =
  match e with
    ...
    | Match (e1,lst) -> (fv e1)@(List.concat (List.map 
          (fun (i,vs,e)-> diff (fv e) (remove_anon vs)) lst))
    | Let (lst,_,body) -> 
