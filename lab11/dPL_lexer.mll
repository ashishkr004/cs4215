...
= struct
 ...
 let dPL_keywords = Hashtbl.create 100

 let _ = List.map (fun ((k,t):(string*dPL_token)) -> Hashtbl.add dPL_keywords k t)
	[("int", INT_TYP);
   ("bool", BOOL_TYP);
   ("true", TRUE);
   ("false", FALSE);
  ...
  let upper_char = ['A'-'Z']
  let first_upper = upper_char identchar*
...
rule tokenizer file_name = parse
  ...
  | ';' {SEMICOLON}
  | '`' {BACKQUOTE}
  | open_comment { comment file_name lexbuf }
  | first_upper as str {UIDENTIFIER str}
  | ident as idstr
	  {
	    try Hashtbl.find dPL_keywords idstr
...
and comment file_name = parse
    close_comment { tokenizer file_name lexbuf }
