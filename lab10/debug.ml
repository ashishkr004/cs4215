...
struct
  ...
  let remove_anon xs = List.filter (fun i->not(i="_")) xs
 ...
 let pr_list_space f xs = pr_lst " " f xs
 ...
 let add_str s f xs = s^":"^(f xs)
  ...
  struct 
    ...
    let string_of (pr:'b->string) (env:'b et) : string =
      pr_list (pr_pair pr_id pr) env
  end;;
