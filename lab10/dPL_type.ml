...
" pr_subs) s2;
	...
	| RecFunc (te,id,args,body) -> 
	    ...
	    let env_ls = List.map (fun v -> (v, fresh_tv())) (id::args) in
	    let ((s,bt), nbody) = infer (env_ls@env) body in
	    (* and _ = List.iter (fun (x,y) -> print_endline (x^" gives "^(string_of_dPL_type y))) env_ls *)
	    (* and _ = print_endline (string_of_dPL_type bt) in *)
	    let at = List.map (fun (_,t) -> apply_subs s t) env_ls in
	    (* let _ =  List.iter (fun x -> print_endline (string_of_dPL_type x)) at in *)
	    let nft = create_fn_type_bare at bt in
	    (* let _ = print_endline (string_of_dPL_type nft) in *)
	    let nbody = apply_type_subs ((id,nft)::s) nbody in
	    (((id,nft)::s,nft), RecFunc(Some nft, id, args, nbody))
    in 
...
let type_infer (env:env_type) (e:dPL_expr) : outcome * dPL_expr =
    ...
    Debug.ho_2 "type_infer" (Environ.string_of string_of_dPL_type) pr pr_out type_infer env e
