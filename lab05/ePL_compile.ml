...
let compile (e:ePL_expr) : eVML_inst list =
  match EPL_type.type_infer e with
    | None -> failwith "expression failed type check"
    | Some t -> match EPL_type.type_check e t with
        | false -> failwith "expression failed type check"
        | true ->
          let rec helper e =
            match e with
              | IntConst i -> [LDCI i]
              | BoolConst b -> if b = true then [LDCB 1] else [LDCB 0]
              | UnaryPrimApp (op,arg) ->
                let s = helper arg in
                s@[trans_cmd op]
              | BinaryPrimApp (op,arg1,arg2) ->
                let s1 = helper arg1
                and s2 = helper arg2 in
                s1@s2@[trans_cmd op]
          in
          (helper e)@[DONE]
